function [ orientation_array, fitT] = orientation_5_MOD( Data, reference, minN, symmineqidx)
%NAME: orientation
%FUNCTION: 
...
fitT = zeros(1,n); 
%oto_all_check = zeros(1,n); %MOD HERE 
indecesT = generate_indeces(minN, maxN, dimension);
U = cell(n, 1); %V5 addition April 2019
parfor a = 1:n %V5 addition April 2019 %parfor here
    cent = Data(a).pos; 
    ...
    [Ar, Br, fit, ~] = register_points_sumoutput_4_loop_MOD(rpoints, reference, dimension, indeces, symmineqidx);
    sA = rpoints(Ar, :); 
    sB = reference(Br, :); 
    fitT(a) = fit; 
    %oto_all_check(a) = oto_all_checka; 
