function [ Ar, Br, Fit, minDiffSq ] = register_points_sumoutput_4_loop_MOD( A,B, dimension, indeces, symmineqidx)
...
%NUD = zeros(1,numel(symmineqidx)/ dimension); 
IND = zeros((numel(symmineqidx)/ dimension) * npointsA,2);
...
for b = 1:numel(symmineqidx)/ dimension
    ...
    %UD_1 = unique(indecesD_sorted(1:npointsA, 1)); %can remove this after test  
    %UD_2 = unique(indecesD_sorted(1:npointsA, 2)); %and this
    ...
    %else
        ...
        IND(b*npointsA - npointsA + 1: b*npointsA, :)...
            = indecesD_sorted(1:npointsA, :);  
        %crash
        %if numel(UD_1) < npointsA || numel(UD_2) <npointsA
            %NUD(1,b) = 0; %if not one to one. 
        %else 
            %NUD(1,b) = 1;
        %end
    %end 
...
mindimselect = min(minDiffSq); %The minimum sum regardless of one to one
IDIN = find(minDiffSq == mindimselect);
IND_to_check = IND(IDIN*npointsA - npointsA +1 :IDIN *npointsA,:);
UD_1 = unique(IND_to_check(:,1)) ;
UD_2 = unique(IND_to_check(:,2)) ;
%crash
%oto_check = NUD(minDiffSq == mindimselect); 
if numel(UD_1) < npointsA || numel(UD_2) <npointsA
    Fit = -1; 
    Ar = 1:npointsA; 
    Br = 1:npointsA; %so dont have to use a padded kabsch calc
else
    registration2i = registration1((minDiffSq == min(minDiffSq))); 
    registration2 = registration2i{1}; 
    Fit = mindimselect/ npointsA; 
    ArBr = registration2;
    Ar = ArBr(:,1); 
...
% if oto_check == 0
%     %oto_all_check = sum(NUD); %this value will be how many are oto. 
%     Fit = -1; 
%     Ar = 1:npointsA; 
%     Br = 1:npointsA; %so dont have to use a padded kabsch calc
% else 
%     %oto_all_check = -1; %this means that the original was registered w oto
%     registration2i = registration1((minDiffSq == min(minDiffSq))); 
%     registration2 = registration2i{1}; 
%     Fit = mindimselect/ npointsA; 
%     ArBr = registration2;
