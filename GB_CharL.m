function [BptsRF,con,distedgesB] = GB_CharL(G, xyzqs, qOperatorCell, minmaxx, minmaxy, minmaxz, minL, maxL, mode)
%NAME: GB_CharL
%FUNCTION: Characterise the grain boundaries and create the mesh
%connectivity. This function can also find intra-grain misorientation. 
%INPUTS: 
...
%   minL - minimum distance two grains may be apart. 
%   maxL - maximum distance two grains may be apart. May be set small to
%   find misorientations within grains. 
%   mode - set to 1 if you want to calculate the misorientation between
%   particles in different grains. Set to 2 if you want all misorientations
%   of the delaunay triangulation to be recorded. Mode 2 is useful
%   if you want to know what misorientations there are within grains.
%   Correspondingly set the minL and maxL to 0 and Rcut (1st minimum g of
%   r)
% 
%OUTPUTS: 
...
%   distedgesB - the lengths between the grains that are bisected with a
%   boundary.
%   neighbours - 
%HISTORY: 
%Written by Nick Orr April 2020 in response to finding bugs in the previous
%versions of this code. 
%March 2021, added a function to add an upper bound to the distance between
%grains for GB assignment 
...
if mode == 1
    IGedgesA = DTedges(DTedgesG_id(:,1) ~= DTedgesG_id(:,2),:); %intergrain edges
    IGedges_idA = DTedgesG_id(DTedgesG_id(:,1) ~= DTedgesG_id(:,2),:);
elseif mode == 2 %  This makes all particles thier own grain.
    IGedgesA = DTedges;
    IGedges_idA = DTedgesG_id;
    %crash
end
%find the lengths of all the edges. 
distedges = sqrt(sum((xyzqsG(IGedgesA(:,1), 1:3)- xyzqsG(IGedgesA(:,2), 1:3)).^2,2)); 
IGedges = IGedgesA(distedges < maxL & distedges > minL, :); 
IGedges_id = IGedges_idA(distedges < maxL & distedges > minL, :); 
distedges2 = distedges(distedges < maxL & distedges > minL, :);
%crash 
